...
export function hasQueryBuilder(arg: any): arg is IQuery {
  return (
    ...
    typeof arg === 'object' &&
    'query' in arg &&
    arg.query instanceof QueryBuilder
  );
}
...
export class QueryBuilder {
  ...
  public prependConstructor<T extends string>(
    name: T,
  ...
  public appendCall<T extends string>(
    name: T,
    ...args: QueryCallArgument[]
  ): QueryBuilder {
  ...
  public build(
    argumentsArr: QueryCallArgument[] = [],
    skip: number = 0
  ): IQueryCall {
  ...
  private subQueryScriptWithArguments(
  ...
  private buildSubQuery(
    ...
    if (hasQueryBuilder(subQuery)) {
      ...
      result.reduce((acc, val) => {
        acc.push(
          '  ' +
            this.buildSubQuery(parentQueryVar, val, args, buildFunction, skip)
        );
        return acc;
      }, lines);
