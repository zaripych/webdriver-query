...
export class QueryError extends Error {
  ...
  private static collectMessage(message: string, inner?: Error | QueryError): string {
    const delimiter = ' --> '
    ...
    while (current) {
      msg = QueryError.appendInnerText(msg, current, indent, delimiter, getErrorNameAndMessage)
      indent += 1
  ...
  constructor(message: string, inner?: Error, query?: QueryBuilder, additional?: string)
  constructor(
    ...
    if (typeof message === 'string') {
      ...
      const blocks = [
        message,
        query ? `, when executing a sub-query:\n\n    ${query.buildDescription(1)}\n` : '',
        additional || '',
      ]
