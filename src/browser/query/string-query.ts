...
import { QueryBuilder, IStringQuery, INumberConversionOptions } from '../../shared/query'
...
export class StringQuery extends BaseQuery<string> implements IStringQuery {
      ...
      this.build(() => {
        return this.perform().then(resolvedValue => {
          if (typeof expression === 'string') {
            return resolvedValue !== null && new RegExp(expression).test(resolvedValue)
          } else {
  ...
  private appendCall(name: keyof IStringQuery, ...args: Array<{}>): QueryBuilder {
