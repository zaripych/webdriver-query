import { IBaseQuery, isQuery, IQueryConstructor, BaseQuery } from './base-query'
...
function switchWithValidation<This, Q extends IBaseQuery<R>, R = any>(
  ...
  if (!isQuery(test)) {
    throw new ArgumentError('The first argument function should return a query')
  }
  ...
  if (isQuery(arg2Result)) {
    if (!arg3) {
      throw new Errors.ArgumentError(
        'When first argument is a condition query the third argument' + 'is required'
      )
    }
    ...
    if (!isQuery(falsy)) {
      throw new ArgumentError('The third argument function should return a query')
    }

    const constructor: any =
      truthy.constructor === falsy.constructor ? truthy.constructor : AnyQuery
    return switchBooleanTo(query, buildingBlock, constructor, test, truthy, falsy)
    ...
    keys.forEach(key => {
      if (!isQuery(options[key])) {
        throw new Errors.ArgumentError(`The option with value ${key} doesn't resolve to a query`)
      }
    })
    ...
    if (whenNoMatch !== undefined && !isQuery(whenNoMatch)) {
      throw new ArgumentError('The last argument function should return a query')
    }
    ...
    if (keys.length === 0) {
      ...
      if (noMatchConstructor) {
        constructor =
          noMatchConstructor === values[0].constructor ? values[0].constructor : AnyQuery
      } else {
        constructor = values[0].constructor
      }
    }

    return switchStringOrNumberTo(query, buildingBlock, constructor, test, options, whenNoMatch)
  } else {
...
Query.prototype.switch = function switchOverloaded<Q extends IBaseQuery<Qr>, Qr = any>(
  this: Query,
  ...
  return switchWithValidation(this, query, this.buildingBlock, arg0, arg1, arg2)
}
...
BaseQuery.prototype.switch = function switchForAllQueries<T>(
  ...
  const query = this.buildingBlock.query.appendCall('switch', ...args)
  return switchWithValidation(this, query, this.buildingBlock, () => this, arg1, arg2)
}
