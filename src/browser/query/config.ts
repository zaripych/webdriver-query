...
export const validateConfig = (config: IConfig) => {
  ...
  if (typeof config.waitTimeoutMilliseconds !== 'number') {
    throw new Errors.ArgumentError(`'waitTimeoutMilliseconds' is expected to be a positive number`)
  }
  ...
  if (typeof config.pollTimes !== 'number') {
    throw new Errors.ArgumentError(`'pollTimes' is expected to be a positive number`)
  }
  if (config.minPollPeriodMilliseconds > config.waitTimeoutMilliseconds) {
    throw new Errors.ArgumentError(
      `'minPollPeriodMilliseconds' (${config.minPollPeriodMilliseconds}ms) cannot` +
        ` be greater than 'waitTimeoutMilliseconds' (${config.waitTimeoutMilliseconds}ms)`
    )
  }
  if (config.pollTimes < 2) {
    throw new Errors.ArgumentError(`'pollTimes' (${config.pollTimes}) cannot` + ` be less than 2`)
  }
}
