...
import { ArrayQuery, createArrayQueryForSingleElementQuery, createArrayQuery } from './array-query'
...
import { LocationQuery, SizeQuery, ObjectQuery, RectQuery } from './object-query'

export class ElementQuery extends BaseQuery<HTMLElement> implements IElementQuery {
  public findElements(selector: Selector): ArrayQuery<HTMLElement, ElementQuery> {
    const query = this.appendCall('findElements', selector)
    return createArrayQueryForSingleElementQuery(
      this,
      this.build(() => {
        return this.perform().then(e => this.library.select(selector, e, query))
      }, query)
    )
  }
  ...
  public getText(): StringQuery {
    ...
    return new StringQuery(
      this.build(() => {
        return this.perform().then(e => (this.library.isDisplayed(e) ? this.library.textOf(e) : ''))
      }, query)
    )
  }
  ...
  public getCssValue(cssStyleProperty: string): StringQuery {
    ...
    return new StringQuery(
      this.build(() => {
        return this.perform().then(e => this.library.cssOf(e, cssStyleProperty))
      }, query)
    )
  }
  ...
  public imitateSelection(options: SelectOption | SelectOption[]): ElementQuery {
  ...
  public getSelectedOptions(): ArrayQuery<ISelectedOption, ObjectQuery<ISelectedOption>> {
  ...
  private appendCall(name: keyof IElementQuery, ...args: Array<{}>): QueryBuilder {
