...
declare module './query' {
  ...
  interface Query {
    firstTruthy(queries: StringQuery[] | ((q: this) => StringQuery[])): StringQuery
    firstTruthy(
      queries: NullableStringQuery[] | ((q: this) => NullableStringQuery[])
    ): NullableStringQuery
    firstTruthy(queries: ElementQuery[] | ((q: this) => ElementQuery[])): ElementQuery
    firstTruthy(queries: NumberQuery[] | ((q: this) => NumberQuery[])): NumberQuery
    firstTruthy(queries: ConditionQuery[] | ((q: this) => ConditionQuery[])): ConditionQuery
    firstTruthy<R, Qr extends IBaseQuery<R>>(queries: Qr[] | ((q: this) => Qr[])): AnyQuery
  }
}
...
Query.prototype.firstTruthy = function first<R, Qr extends IBaseQuery<R>>(
  ...
  const queryFactory = (): Promise<R | AnyResultType> => {
    ...
    const iterate = (): Promise<AnyResultType> => {
      if (index < 0 || index >= subQueries.length) {
        return Promise.reject(
          new OutOfOptionsError(
            'None of the sub-queries resolved' + ' or all of them resolved as falsy',
            query
          )
        )
      }
      return subQueries[index]
        .perform()
        .catch(err => {
          this.buildingBlock.logger.debug(() => [`firstTruthy, ignoring error at ${index}`, err])
          index++
          return iterate()
        })
        .then(result => {
  ...
  if (subQueries.every(item => item.constructor === firstType)) {
    ...
    return new constructor(this.buildingBlock.build<R>(queryFactory as () => Promise<R>, query))
  } else {
    return new AnyQuery(
      this.buildingBlock.build(queryFactory as () => Promise<AnyResultType>, query)
    )
  }
}
