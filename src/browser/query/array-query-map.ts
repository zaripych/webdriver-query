...
import { ArrayQuery, createArrayQueryForSingleElementQuery } from './array-query'
import { ResultOfQuery } from './result-of-query'

declare module './array-query' {
  // tslint:disable-next-line:interface-name
  interface ArrayQuery<E, Eq> {
    map<T extends IBaseQuery<any>>(query: (q: Eq) => T): ArrayQuery<ResultOfQuery<T>, T>
  }
}
      ...
      return this.perform().then(items => {
        ...
        const iterate = (): Promise<any[]> => {
          ...
          const itemQuery = this.itemQueryFactory(
            this.buildingBlock.build(() => Promise.resolve(items[index]), itemQueryBuilder)
          )
