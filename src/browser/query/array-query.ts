...
> {
  new (
    block: IQueryBuildingBlock<E[]>,
    itemQueryFactory: QueryFactory<E, Eq>
  ): ArrQ;
}
...
> extends BaseQuery<E[]> {
  ...
  constructor(
    buildInfo: IQueryBuildingBlock<E[]>,
    itemQueryFactory: QueryFactory<E, Eq>
  ) {
    super(buildInfo);
        ...
        return this.perform().then(elements => {
          if (!Number.isSafeInteger(index)) {
            return Promise.reject(
              new ArgumentError(`index (${index}) is not an integer`)
            );
            ...
            return Promise.reject(
              new ArgumentError(
                `index (${index}) is out of bounds (${elements.length})`
              )
            );
          }
        });
        ...
        return this.perform().then(items => {
          ...
          const iterate: () => Promise<E> = () => {
            if (index < 0 || index >= items.length) {
              return Promise.reject(
                new NoSuchElementError('No items found matching the condition')
              );
            }
...
>(query: T, buildingBlock: IQueryBuildingBlock<R[]>): ArrayQuery<R, T> {
  if (query instanceof ArrayQuery) {
    ...
    const factory = (block: IQueryBuildingBlock<R>) => {
      const queryConstructor: IQueryConstructor<
        R,
        T
      > = query.constructor as any;
