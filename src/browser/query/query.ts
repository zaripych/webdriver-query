...
export class Query implements IQueryDriver {
  ...
  constructor(arg?: Partial<IConfig> | Partial<IQueryBuildingBlock<void>>) {
    ...
    this.buildingBlock = new QueryBuildingBlock({
      executor: (buildingBlock && buildingBlock.executor) || (() => Promise.resolve()),
      query:
        (buildingBlock && buildingBlock.query) ||
        new QueryBuilder().appendCall(QueryBuilder.constructorName, effectiveConfig),
  ...
  public findElements(selector: Selector): ArrayQuery<HTMLElement, ElementQuery> {
    ...
    return createArrayQuery(
      this.build(() => {
        return this.whenReady().then(() => this.buildingBlock.library.select(selector, null, query))
      }, query),
    ...
    return new ElementQuery(
      this.build(() => {
        return this.whenReady().then(() =>
          Promise.resolve(this.buildingBlock.library.selectFirst(selector, null, query))
        )
      }, query)
    )
  }
  ...
  public sequence<I extends OneOfQueries, T extends I[]>(queries: (q: this) => T): Query {
  ...
  private build<Y>(executor: () => Promise<Y>, query: QueryBuilder): IQueryBuildingBlock<Y> {
