...
export class ArrayQuery<
  T,
  TItemQuery = QueryForResultType<T>
> extends PromiseLikeQuery<T[]> {
  public readonly itemQueryFactory: QueryFactory<TItemQuery>;

  constructor(
    buildInfo: IQueryBuildingBlock,
    itemQueryFactory: QueryFactory<TItemQuery>
  ) {
    /* istanbul ignore next */
    if (!itemQueryFactory) {
      throw new ArgumentError(
        'itemQueryFactory is mandatory parameter for BaseArrayQuery'
      );
    }
  ...
  public filter(condition: (q: TItemQuery) => ConditionQuery): this {
    const query = this.query.appendCall(
      'filter',
      new SubQueryInfo(this.at(0) as any, condition as any)
    );
  ...
  public map<R extends IBaseQuery<any>>(
    itemsQuery: (q: TItemQuery) => R
  ): ArrayQuery<ResultOfQuery<R>, R> {
    const firstQuery = this.at(0);
    const mapQuery = itemsQuery(firstQuery);
    if (!mapQuery.constructor) {
      throw new ArgumentError(
        'The query returned by map function doesn\'t have a constructor'
      );
    }
    ...
    const query = this.query.appendCall(
      'map',
      new SubQueryInfo(firstQuery as any, itemsQuery as any)
    );
    return new ArrayQuery(
      this.build(query),
      build => new mapQueryConstructor(build)
    );
  }

  public create(buildingBlock: IQueryBuildingBlock): this {
    const constructor = this.constructor as IArrayQueryConstructor<
      T,
      TItemQuery,
      this
    >;
